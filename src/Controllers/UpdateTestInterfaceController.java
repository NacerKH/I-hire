    ...
    private void saveExamen(ActionEvent event) {
        ...
        if (fxErrorTopic.getText().trim().isEmpty() && fxErrorTotalQuestions.getText().trim().isEmpty()) {
            if (Integer.valueOf(fxTotalQuestions.getText()) > test.getQuestions().size()) {
                ...
                AlertInterface.showAlertWithHeaderText(stage, "le nombre de questions doit être supérieur ou égal aux nombre de questions a sélectionné, Merci d'ajouter des questions avant de le modifier.");
            }
            else
            {
